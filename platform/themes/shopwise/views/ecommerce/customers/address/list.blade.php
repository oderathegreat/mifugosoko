                        ...
                        @foreach($addresses as $address)
                            <tr>
                                <td style="white-space: inherit;">
                                    <p>{{ $address->name }}, {{ $address->address }}, {{ $address->city }}, {{ $address->state }}, {{ $address->country_name }} - {{ $address->phone }}</p>
                                </td>
                                <td style="width: 120px;">
                                    @if ($address->is_default) {{ __('Yes') }} @else {{ __('No') }} @endif
                                </td>
                                <td style="width: 100px;">
                                    <a class="btn btn-fill-out btn-sm" href="{{ route('customer.address.edit', $address->id) }}">{{ __('Edit') }}</a>
