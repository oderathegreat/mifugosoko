...
<div class="next-form-section">
        ...
        <div class="next-form-grid-cell">
            <div class="table-wrap p-none">
                <table class="table border-bottom mb0">
                    ...
                    <tr>
                        ...
                        <th class="text-center">{{ __('Restock quantity') }}</th>
                        <th class="text-center">{{ __('Remain') }}</th>
                    </tr>
                    </thead>
                    <tbody>
            ...
            <div class="table-wrap p-none">
                <table class="refund-totals">
                        ...
                        <tr>
                            <td class="text-right" colspan="2">
                                <label class="next-label inline mt10">
                                    ...
                                    <span>
                                        {{ __('Return') }} <span class="total-restock-items">{{ $order->products->sum('qty') - $order->products->sum('restock_quantity') }}</span> {{ __('product(s)') }}
                                    </span>
                                </label>
                            </td>
                        </tr>
                    @endif
                    <tr>
                        <td class="text-right">{{ __('Shipping fee') }}</td>
                    ...
                    @if ($order->payment->refunded_amount)
                        <tr>
                            <td class="text-right">{{ __('Total refund amount:') }}</td>
                    ...
                    @endif
                    <tr>
                        <td class="text-right">{{ __('Total amount can be refunded:') }}</td>
                        <td class="text-right quantity text-no-bold">
    ...
    <div class="next-form-grid">
        <div class="next-form-grid-cell">
            <label class="text-title-field" for="refund-note">{{ __('Refund reason (optional)') }}</label>
            <div>
