<form class="shipment-create-panel" action="{{ route('orders.create-shipment', $order->id) }}">
    <div class="pd-all-20 pt10">
        <div class="flexbox-grid-form flexbox-grid-form-no-outside-padding">
            <div class="flexbox-grid-form-item rps-no-pd-none-r">
                <div class="form-group">
                    <label class="text-title-field">{{ __('Warehouse') }}:</label>
            ...
            <div class="flexbox-grid-form-item max-width-220-px">
                <div class="form-group">
                    <div class="form-group">
                        <label class="text-title-field">{{ __('Weight (:unit)', ['unit' => ecommerce_weight_unit()]) }}:</label>
        ...
        <div class="address-header p-b5">
            <div class="left">{{ __('Shipping address') }}:</div>
            <div class="right"><a href="#" class="hover-tooltip hover-underline color-blue btn-trigger-update-shipping-address">{{ __('Edit') }}</a></div>
        </div>
                    ...
                    <div class="user-control-combox-v3" id="select-shipping-provider">
                        <div class="flexbox-grid-default" data-toggle="dropdown" aria-expanded="false">
                            <div class="flexbox-content p-none-important">
                                ...
                                <input type="text" readonly="readonly" placeholder="{{ __('Select shipping method') }}" class="next-input input-dropdown input-show-shipping-method" value="{{ $order->shipping_method_name }}">
                            </div>
                        </div>
                        <div class="dropdown-menu dropdown-carrier font-size-13px animate-scale-dropdown bg-white">
                            ...
                            <div>
                                <table class="table table-fix-header2 table-shipping-select-options">
                                    <thead>
                                    <tr>
                                        <th><b>{{ __('Package') }}</b><i style="display:block;font-size: xx-small; color: darkgoldenrod"><span class="ws-nm">{{ __('Warehouse') }} {{ get_ecommerce_setting('store_city') }}, {{ get_ecommerce_setting('store_state') }}</span></i></th>
                                        <th class="text-right"><b>{{ __('Shipping fee/COD') }}</b><i style="display:block;font-size: xx-small;">{{ __('Fee') }}</i></th>
                                    </tr>
                                    </thead>
                                    <tbody>
            ...
            <div class="flexbox-grid-form flexbox-grid-form-item flexbox-grid-form-no-outside-padding rps-no-pd-none-l ps-relative max-width-220-px">
                <div class="flexbox-grid-form-item">
                    @if ($order->payment->payment_channel == \Botble\Payment\Enums\PaymentMethodEnum::COD && $order->payment->status !== \Botble\Payment\Enums\PaymentStatusEnum::COMPLETED)
                        <div class="form-group">
                            <label class="text-title-field">{{ __('Cash on delivery amount (COD)') }}:</label>
        ...
        <div class="flexbox-grid-default flexbox-align-items-center">
            <div class="flexbox-auto-content text-left">
                <label class="next-label">
                    ...
                    <span class="pre-line">{{ __('Send confirmation email to customer') }}</span>
                </label>
            </div>
            <div class="flexbox-auto-content">
                <button type="button" class="btn btn-secondary btn-close-shipment-panel">{{ __('Cancel') }}</button>
                <button type="button" class="btn btn-primary btn-create-shipment">{{ __('Finish') }}</button>
            </div>
        </div>
    </div>
</form>
